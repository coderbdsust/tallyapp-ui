...
export class CardPageVisitsComponent extends PaginatedComponent<Transaction> implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.orgService.organization$.subscribe((org) => {
            if (org) {
                ...
                this.loadTransactionSummary(this.transactionType, this.startDate, this.endDate, this.currentPage, this.selectedRows);
            }
        });
  ...
  loadTransactionSummary(transactionType: string = '', startDate: string | null = null, endDate: string | null = null, page: number = 0, size: number = 10) {
    if (this.organization) {
      this.accService.getTransactionSummaryByPage(this.organization.id, transactionType, startDate, endDate, page, size).subscribe({
        next: (response) => {
